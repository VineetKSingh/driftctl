...
// interfaces below are only used to be able to create mock on azure sdk
...
func (s *storageRepository) ListAllStorageAccount() ([]*armstorage.StorageAccount, error) {
	...
	// Since ListAllStorageAccount can be called from multiples providers we should lock here to ensure
	// the cache is hit when multiples calls are running in parallel
